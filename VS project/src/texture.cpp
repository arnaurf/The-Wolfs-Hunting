...
Vector4 Image::getPixelInterpolatedHigh(float x, float y, bool repeat) {
	...
	int ix2 = ix < width - 1 ? ix + 1 : 0;
	int iy2 = iy < height - 1 ? iy + 1 : 0;
	Vector4 c1 = getPixel(ix, iy).toVector4();
	Vector4 c2 = getPixel(ix2, iy).toVector4();
	Vector4 c3 = getPixel(ix, iy2).toVector4();
	Vector4 c4 = getPixel(ix2, iy2).toVector4();
	Vector4 top = lerp( c1, c2, fx );
	Vector4 bottom = lerp( c3, c4, fx );
	return lerp(top, bottom, fy);
};
